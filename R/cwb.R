...
cwb_install <- function(url_cwb = cwb_get_url()){
  ...
  tmp_dir <- tempdir()
  download.file(url_cwb, destfile = file.path(tmp_dir, basename(url_cwb)))
  
  if (.Platform$OS.type == "windows"){
    unzip(file.path(tmp_dir, basename(url_cwb)))
    install_script_file <- file.path(tmp_dir, subdir, "install-cwb-win.bat")
    ...
    # setwd(file.path(tmp_dir, subdir))
    shell(install_script_file)
  } else {
    untar(file.path(tmp_dir, basename(url_cwb)), exdir = tmp_dir)
    install_script_file <- file.path(tmp_dir, subdir, "install-cwb.sh")
    install_script <- readLines(install_script_file)
    ...
    install_script <- gsub("bin/\\*", sprintf("%s/*", file.path(tmp_dir, subdir, "bin")), install_script)
    install_script <- gsub("bin/cwb-config", file.path(tmp_dir, subdir, "bin", "cwb-config"), install_script)
    install_script <- gsub("instutils/cwb-config.in", file.path(tmp_dir, subdir, "instutils", "cwb-config.in"), install_script)
    install_script <- gsub("instutils/install.sh", file.path(tmp_dir, subdir, "instutils", "install.sh"), install_script)
    install_script <- gsub("lib/libcl.a", file.path(tmp_dir, subdir, "lib", "libcl.a"), install_script)
    install_script <- gsub("include/cwb/cl.h", file.path(tmp_dir, subdir, "include", "cwb", "cl.h"), install_script)
    install_script <- gsub("include/cwb/cqi.h", file.path(tmp_dir, subdir, "include", "cwb", "cqi.h"), install_script)
    install_script <- gsub("man/man1/*", file.path(tmp_dir, subdir, "man", "man1", "*"), install_script)
    cat(install_script, file = install_script_file, sep = "\n")
